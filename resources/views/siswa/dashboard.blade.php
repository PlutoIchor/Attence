    ...
    <table class="table" style="table-layout: fixed">
        ...
        <tbody>
            @foreach ($jadwalHariIni as $jadwal)
                ...
                $validasi = $jadwal->validations
                    ->where('id_siswa', '=', $data_siswa->id)
                    ->where('created_at', '<', $waktuBerakhir, 'AND', 'created_at', '>', $waktuMulai)
                    ->first();
                
                $absen = $jadwal->attendances
                    ->where('id_siswa', '=', $data_siswa->id)
                    ->where('created_at', '<', $waktuBerakhir, 'AND', 'created_at', '>', $waktuMulai)
                    ->first();
                ?>
                <tr>
                    ...
                    @if (date('H:i:s') >= $jadwal->waktu_mulai && date('H:i:s') <= $jadwal->waktu_berakhir && !$absen)
                        ...
                        @if ($validasi)
                            <td scope="col" colspan="2" class="text-secondary">-- Menunggu untuk diaccept --</td>
                        @else
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <form
                                                    action="{{ url('/absenSekarang/' . $jadwal->id . '/' . $data_siswa->id) }}"
                    ...
                    @else
                        ...
                        @else
                            @if (!$absen)
                                ...
                                @endif
                            @else
                                <td scope="col" colspan="2" class="text-success">{{ $absen->keterangan }}</td>
                            @endif
                        @endif
                    @endif
                </tr>
